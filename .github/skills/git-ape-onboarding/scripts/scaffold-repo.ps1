...
.DESCRIPTION
    PowerShell parity for scaffold-repo.sh.
...
.PARAMETER TargetRepoRoot
    The target repository root to scaffold into.
...
.EXAMPLE
    pwsh .github/skills/git-ape-onboarding/scripts/scaffold-repo.ps1 C:\path\to\repo
#>
...
$mappings = @(
    @{ Src = 'workflows/git-ape-plan.yml';        Dst = '.github/workflows/git-ape-plan.yml' }
    @{ Src = 'workflows/git-ape-deploy.yml';      Dst = '.github/workflows/git-ape-deploy.yml' }
    @{ Src = 'workflows/git-ape-destroy.yml';     Dst = '.github/workflows/git-ape-destroy.yml' }
...
Write-Host "Scaffolding Git-Ape files into: $TargetRepoRoot" -ForegroundColor White
Write-Host ""
    ...
    foreach ($path in $skippedPaths) {
        switch -Regex ($path) {
            '^\.github/copilot-instructions\.md$' {
                $srcRel = 'copilot-instructions.md'
            }
            '^\.github/workflows/' {
...
Write-Host ""
Write-Host "Files were left UNSTAGED. Review them, then commit when ready."
