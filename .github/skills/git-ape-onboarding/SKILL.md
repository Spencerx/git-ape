...
description: "Onboard Git-Ape at two layers: (1) a repository + Azure subscription(s) + user identity for CI/CD (OIDC, federated credentials, RBAC, GitHub environments, secrets), and (2) enterprise-wide distribution via a `.github-private` repo (managed-settings.json plugin standards + custom agents). Use for first-time Git-Ape setup, new subscription onboarding, or rolling Git-Ape out org/enterprise-wide."
metadata:
...
7. Never run `git add`, `git commit`, `git push`, or open a PR for the
   ...
   them.
8. **Enterprise mode:** confirm the target org belongs to the enterprise and the
...
9. **Enterprise mode:** never claim to have automated the **AI-controls
